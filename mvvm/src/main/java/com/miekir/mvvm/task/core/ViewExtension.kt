...
/**
 * @param loadingClazz 自定义的弹窗，不传的话，使用系统默认的弹窗不能直接传对象，否则旋转恢复时取不到之前的实例，强行引用会造成内存泄漏
 */
fun IView.withLoadingDialog(
    loadingType: LoadingType = LoadingType.VISIBLE,
    message: String = "",
    loadingClazz: Class<out TaskLoading> = MvvmManager.getInstance().globalTaskLoadingClass ?: TaskLoading::class.java,
    ...
    val loadingLiveData = MutableLiveData(
        DialogData(
            ...
            taskJob = taskJob,
            loadingClazz = loadingClazz,
        )
    )
    taskJob.setupDialogData(loadingLiveData)
    ...
    basicActivity.lifecycle.addObserver(LifecycleEventObserver { _, event ->
        if (event == Lifecycle.Event.ON_DESTROY) {
            loadingLiveData.removeObservers(basicActivity)
        }
    })
    if (basicActivity.lifecycle.currentState == Lifecycle.State.DESTROYED) {
        loadingLiveData.removeObservers(basicActivity)
    }
}
